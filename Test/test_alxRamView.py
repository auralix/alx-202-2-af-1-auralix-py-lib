"""alxRamView - the firmware RAM view over a scripted gdb and a fake probe (no toolchain, no target).
...
import alxRamView
from alxRamView import RamView, decode, find_gdb
...
class FakeGdb:
    ...
    def __call__(self, argv, capture_output, text, timeout):
        ...
        return subprocess.CompletedProcess(argv, 0, stdout=out, stderr="warning: no debugging symbols?\n")
...
class FakeProbe:
    ...
    def read_mem8(self, reads, script_name):
        reads = list(reads)
        self.calls.append((reads, script_name))
...
def test_ALX1544_P50_resolve_once_then_snapshot_in_one_probe_session(elf, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(alxRamView.subprocess, "run", gdb)
    probe = FakeProbe({0x20000010: b"\x01", 0x20000011: b"\x69", 0x20000014: struct.pack("<f", 11.75)})
    view = RamView(probe, elf, VARS, gdb=tmp_path / "arm-none-eabi-gdb.exe")
    assert len(gdb.calls) == 1
    argv = gdb.calls[0]
    assert argv[0] == str(tmp_path / "arm-none-eabi-gdb.exe") and argv[1] == "--batch" and argv[-1] == str(elf)
    assert argv[2:-1] == ["-ex", "print/x &app.inputs.ok", "-ex", "print/x &app.out.pct", "-ex", "print/x &app.meas.v"]
    assert view.addr == ADDRS
    assert view.snapshot() == {"input_ok": True, "out_pct": 105, "volts": 11.75}
    assert probe.calls == [([(0x20000010, 1), (0x20000011, 1), (0x20000014, 4)], "ram.jlink")]
    assert len(gdb.calls) == 1, "addresses are resolved once, not per snapshot"


def test_ALX1544_P51_partial_symbol_resolution_raises_with_the_gdb_output(elf, tmp_path, monkeypatch):
    monkeypatch.setattr(alxRamView.subprocess, "run", FakeGdb([0x20000010, 0x20000011]))
    with pytest.raises(RuntimeError, match="gdb resolved 2 of 3 symbols") as ex:
...
def test_ALX1544_P52_missing_elf_or_gdb_raises_before_running_anything(elf, tmp_path, monkeypatch):
    ran = FakeGdb([])
    monkeypatch.setattr(alxRamView.subprocess, "run", ran)
    with pytest.raises(RuntimeError, match="ELF of the flashed build not found"):
        RamView(FakeProbe({}), tmp_path / "nope.elf", VARS, gdb=tmp_path / "gdb")
    monkeypatch.setattr(alxRamView, "find_gdb", lambda: None)
    with pytest.raises(RuntimeError, match="arm-none-eabi-gdb not found"):
...
@pytest.mark.parametrize("raw,fmt,value", [
    (b"\x69", "u8", 105), (b"\xff", "i8", -1),
    (b"\x00", "bool", False), (b"\x01", "bool", True), (b"\x02", "bool", True),
    (b"\x34\x12", "u16", 0x1234), (b"\xff\xff", "i16", -1),
    (b"\x78\x56\x34\x12", "u32", 0x12345678), (b"\xfe\xff\xff\xff", "i32", -2),
    (struct.pack("<f", 12.345), "f32", 12.35), (struct.pack("<f", 0.0), "f32", 0.0),
], ids=lambda x: x if isinstance(x, str) else None)
def test_ALX1544_P54_decode_little_endian_formats(raw, fmt, value):
    assert decode(raw, fmt) == value
    assert decode(raw + b"\xaa\xaa\xaa\xaa", fmt) == value, "extra bytes after the value are ignored"
...
def test_ALX1544_P55_find_gdb_order_env_path_known_locations(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(alxRamView.shutil, "which", lambda name: str(tmp_path / "onpath" / name))
    assert find_gdb() == tmp_path / "onpath" / "arm-none-eabi-gdb"
    monkeypatch.setattr(alxRamView.shutil, "which", lambda name: None)
    monkeypatch.setattr(alxRamView, "GDB_CANDIDATES", (tmp_path / "missing" / "gdb.exe", gdb))
    assert find_gdb() == gdb
    monkeypatch.setattr(alxRamView, "GDB_CANDIDATES", (tmp_path / "missing" / "gdb.exe",))
